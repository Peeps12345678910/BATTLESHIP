#########################################################
# File Name: BATTLEPLANE.py    
# Program: 
# Author: Philip and Carol
# Date: June 12 2024
# Description: 
...
class plane:
    def __init__(self, name, img, pos, size, numweapons=0, weaponPath=None, weaponsize=None, weaponCoordsOffset=None):
        self.name = name
        self.pos = pos
        ...
        self.rotation = False
        #  plane is current selection
        self.active = False
        #  Load weapon Images
        self.weaponslist = []
        if numweapons > 0:
            self.weaponCoordsOffset = weaponCoordsOffset
            for num in range(numweapons):
                self.weaponslist.append(
                    weapons(weaponPath,
                         self.rect.center,
                         (size[0] * weaponsize[0],
                          size[1] * weaponsize[1]),
                         self.weaponCoordsOffset[num])
                )


    def selectplaneAndMove(self):
        """Selects the plane once the player click on it and moves the according to the mouse position"""
    ...
    def rotateplane(self, doRotation=False):
        """switch plane between vertical and horizontal"""
    ...
    def checkForCollisions(self, planelist):
        """Check to make sure the plane is not colliding with any of the other planes"""
        slist = planelist.copy()
        slist.remove(self)
        for item in slist:
    ...
    def checkForRotateCollisions(self, planelist):
        """Check to make sure the plane is not going to collide with any other plane before rotating"""
        slist = planelist.copy()
        slist.remove(self)
        for plane in slist:
            if self.rotation == True:
                if self.vImageRect.colliderect(plane.rect):
                    return True
            else:
                if self.hImageRect.colliderect(plane.rect):
                    return True
        return False


    def returnToDefaultPosition(self):
        """Returns the plane to its default position"""
        if self.rotation == True:
            self.rotateplane(True)
    ...
    def snapToGridEdge(self, gridCoords):
        if self.rect.topleft != self.pos:
            ...
            #  Check to see if the planes position is outside of the grid:
    ...
    def draw(self, window):
        ...
        for weapons in self.weaponslist:
            weapons.draw(window, self)
...
class weapons:
    ...
    def update(self, plane):
        """Updating the weapons Positions on the plane"""
        
        if plane.rotation == False:
            self.rect.center = (plane.rect.centerx, plane.rect.centery + (plane.image.get_height()//2 * self.offset))
        else:
            self.rect.center = (plane.rect.centerx + (plane.image.get_width()//2 * -self.offset), plane.rect.centery)
...
class Button:
    ...
    def actionOnPress(self):
        ...
        if self.active:
            if self.name == 'Randomize':
                self.randomizeplanePositions(pFleet, pGameGrid)
                self.randomizeplanePositions(cFleet, cGameGrid)
            elif self.name == 'Reset':
                self.resetplanes(pFleet)
    ...
    def randomizeplanePositions(self, planelist, gameGrid):
        """Calls the randomize planes function"""
        if DEPLOYMENT == True:
            randomizeplanePositions(planelist, gameGrid)


    def resetplanes(self, planelist):
        """Resets the planes to their default positions"""
        if DEPLOYMENT == True:
            for plane in planelist:
                plane.returnToDefaultPosition()
...
def createGameLogic(rows, cols):
    """Updates the game grid with logic, ie - spaces and X for planes"""
    gamelogic = []
    for row in range(rows):
        rowX = []
        for col in range(cols):
...
def updateGameLogic(coordGrid, planelist, gamelogic):
    """Updates the game grid with the position of the planes"""
    for i, rowX in enumerate(coordGrid):
        for j, colX in enumerate(rowX):
            if gamelogic[i][j] == 'T' or gamelogic[i][j] == 'X':
                ...
                gamelogic[i][j] = ' '
                for plane in planelist:
                    if pygame.rect.Rect(colX[0], colX[1], CELLSIZE, CELLSIZE).colliderect(plane.rect):
                        gamelogic[i][j] = 'O'
...
def sortFleet(plane, planelist):
    """Rearranges the list of planes"""
    planelist.remove(plane)
    planelist.append(plane)


def randomizeplanePositions(planelist, gamegrid):
    """Select random locations on the game grid for the battleplanes"""
    placedplanes = []
    for i, plane in enumerate(planelist):
        validPosition = False
        while validPosition == False:
            plane.returnToDefaultPosition()
            rotateplane = random.choice([True, False])
            if rotateplane == True:
                yAxis = random.randint(0, 9)
                xAxis = random.randint(0, 9 - (plane.hImage.get_width()//50))
                plane.rotateplane(True)
                plane.rect.topleft = gamegrid[yAxis][xAxis]
            else:
                yAxis = random.randint(0, 9 - (plane.vImage.get_height()//50))
                xAxis = random.randint(0, 9)
                plane.rect.topleft = gamegrid[yAxis][xAxis]
            if len(placedplanes) > 0:
                for item in placedplanes:
                    if plane.rect.colliderect(item.rect):
                        validPosition = False
                        break
                    else:
                        validPosition = True
...
def pick_random_plane_location(gameLogic):
    validChoice = False
    while not validChoice:
...
def planeLabelMaker(msg):
    """Makes the plane labels to display on the screen"""
...
def displayplaneNames(window):
    """Posting the plane labels to the screen in the correct positions"""
    planeLabels = []
    for plane in ['carrier', 'battleplane', 'cruiser', 'destroyer', 'submarine', 'patrol boat', 'rescue boat']:
        planeLabels.append(planeLabelMaker(plane))
    startPos = 25
    for item in planeLabels:
...
def deploymentScreen(window):
    ...
    #  Draw planes to screen
    for plane in pFleet:
        plane.draw(window)
        plane.snapToGridEdge(pGameGrid)
        plane.snapToGrid(pGameGrid)

    displayplaneNames(window)

    for plane in cFleet:
        # plane.draw(window)
        plane.snapToGridEdge(cGameGrid)
        plane.snapToGrid(cGameGrid)
...
FLEET = {
    'battleship': ['battleship', 'assets/images/ships/battleship/BLACKPLANE.png', (125, 600), (40, 195),
                   4, 'assets/images/ships/battleship/battleshipgun.png', (0.4, 0.125), [-0.525, -0.34, 0.67, 0.49]],
    'cruiser': ['cruiser', 'assets/images/ships/cruiser/YELLOWPLANE.png', (200, 600), (40, 195),
    ...
    'patrol boat': ['patrol boat', 'assets/images/ships/patrol boat/patrol boat.png', (425, 600), (20, 95),
                    0, '', None, None],
    'submarine': ['submarine', 'assets/images/ships/submarine/REDPLANE.png', (350, 600), (30, 145),
                  1, 'assets/images/ships/submarine/submarinegun.png', (0.25, 0.125), [-0.45]],
    'carrier': ['carrier', 'assets/images/ships/carrier/GREYPLANE.png', (50, 600), (45, 245),
                0, '', None, None],
...
cFleet = createFleet()
randomizeplanePositions(cFleet, cGameGrid)

printGameLogic()


#  Loading Game Sounds and Images
MAINMENUIMAGE = loadImage('assets/images/background/AIRSHIP.png', (SCREENWIDTH // 3 * 2, SCREENHEIGHT))
ENDSCREENIMAGE = loadImage('assets/images/background/Carrier.jpg', (SCREENWIDTH, SCREENHEIGHT))
BACKGROUND = loadImage('assets/images/background/SKY.jpg', (SCREENWIDTH, SCREENHEIGHT))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                if DEPLOYMENT == True:
                    for plane in pFleet:
                        if plane.rect.collidepoint(pygame.mouse.get_pos()):
                            plane.active = True
                            sortFleet(plane, pFleet)
                            plane.selectplaneAndMove()
                ...
                for button in BUTTONS:
                    if button.rect.collidepoint(pygame.mouse.get_pos()):
                        if button.name == 'Deploy' and button.active == True:
                            ...
                            SCANNER = True
                            INDNUM = 0
                            BLIPPOSITION = pick_random_plane_location(cGameLogic)
                ...
                if DEPLOYMENT == True:
                    for ship in pFleet:
                        if plane.rect.collidepoint(pygame.mouse.get_pos()) and not plane.checkForRotateCollisions(pFleet):
                            plane.rotateplane(True)
